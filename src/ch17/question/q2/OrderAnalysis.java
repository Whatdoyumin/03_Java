    ...
    public static void main(String[] args) {
        ...
        Map<String, Double> answer2 = orders.stream()
                .collect(Collectors.groupingBy(
                        ...
                        Collectors.summingDouble(Order::getTotalAmount)  // value
                ));
        ...
        String answer3 = orders.stream()
                ...
                .collect(Collectors.groupingBy(
                        product -> product.getCategory(),   // key: 카테고리명
                        Collectors.counting()                       // value: 카테고리별 상품 개수
                ))
                .entrySet().stream()
                .max((e1, e2) -> Long.compare(e1.getValue(), e2.getValue()))
                .map(entry -> entry.getKey()) // Key: 카테고리명
                .orElse("데이터 없음");
