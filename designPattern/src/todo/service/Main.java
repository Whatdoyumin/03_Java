...
public class Main {
    ...
    public static void main(String[] args) {
        ...
        while (true) {
            ...
            try {
                boolean commandFount = false;   // 일치하는 명령어가 있는지 체크
                ...
                for(Map.Entry<Pattern, Consumer<String>> entry : commandHandlers.entrySet()) {
                    ...
                    if (entry.getKey().matcher(input).matches()) {
                        ...
                        commandFount = true;        // 일치하는 명령어가 있었다.
                        break;                      // 추가 검색 X
                    }
                }

                if (!commandFount) {    // 일치하는 명령어가 없었을 경우
                    System.out.println("알 수 없는 명령어입니다. 지원하는 명령어를 입력해주세요.");
                }
